...
def extract_serial_from_qr(qr_code: str, serial_numbers: Set[str]) -> Optional[str]:
    ...
    # Проверяем, содержится ли какой-либо из серийных номеров в QR-коде
...
async def compare_qr_codes(file1_data, file2_data):
    ...
    try:
        ...
        logger.info(f"QR-кодов во втором файле: {len(qr_set2)} из {len(df2)}")
        ...
        # Проверяем наличие разницы в серийных номерах
        series_not_in_qr = series_set - found_series  # Отсутствуют в QR-кодах

        # Подготавливаем результаты
        differences = []
        quantity_diff = ""

        # Проверяем количество найденных и не найденных серийных номеров
        if len(found_series) != len(series_set):
            quantity_diff = f"Количество товаров различается: В первом файле (серийные номера): {len(series_set)} | Найдено в QR-кодах: {len(found_series)}"
            logger.info(quantity_diff)

        # Добавляем отсутствующие серийные номера в результаты
        for serial in series_not_in_qr:
            differences.append(f"Серийный номер: {serial} не найден в отсканированных QR-кодах")

        # Формируем файл с результатами если есть различия
        if differences or quantity_diff:
            ...
            with open(txt_path, "w", encoding="utf-8") as txt_file:
                ...
                if differences:
                    txt_file.write("Серийные номера, отсутствующие в отсканированных QR-кодах:\n")
                    txt_file.write("\n".join(differences))
    ...
    try:
        ...
        try:
            with open(txt_result, "r", encoding="utf-8") as f:
                content = f.read()
                has_quantity_diff = "Количество товаров различается" in content
        except Exception as e:
        ...
        return {
            ...
            "quantity_diff": "Количество товаров различается" if has_quantity_diff else ""
        }
            ...
            <div class="container">
                ...
                <div class="instructions">
                    ...
                    <ul>
                        ...
                        <li>Система найдет серийные номера, которые отсутствуют в QR-кодах</li>
                    </ul>
                </div>
                ...
                async function uploadFiles() {
                    ...
                    try {
                        ...
                        if (result.message) {
                            ...
                            resultDiv.innerHTML = `
                                ${result.txt_url ? `<a href="${SERVER_URL}${result.txt_url}" class="download-link" download>📥 Скачать файл с различиями</a>` : ""}
                            `;
